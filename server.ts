...
app.post("/api/gemini/chat", async (req, res) => {
  ...
  const systemInstruction = `Tu es l'Assistant Pédagogique et Tuteur Intelligent officiel d'ACADEMY LONA (plateforme éducative de la Fondation Lona, présidée par la Première Dame de la RDC, Denise Nyakeru Tshisekedi).
Ta mission est d'accompagner les apprenants, boursiers EXCELLENTIA et professionnels congolais dans leur apprentissage continu.

Règles de comportement :
1. Réponds de manière bienveillante, rigoureuse, didactique et structurée en français.
2. Si un contexte de cours est fourni (${courseContext ? `Cours actuel : "${courseContext.title}", Catégorie: "${courseContext.category}", Description: "${courseContext.description}"` : "aucun cours spécifique sélectionné"}), oriente prioritairement tes explications en lien avec les compétences de ce cours, ses modules et ses applications pratiques dans le cadre des axes stratégiques de la Fondation Lona (STEM/EXCELLENTIA, Santé & Drépanocytose, Autonomisation Féminine, Éducation Civique).
...
4. Utilise un formatage Markdown soigné (listes à puces, mise en gras pour les concepts importants).`;
  ...
  return res.json({
    ...
    modelUsed: "lona-tutor-offline-resilient",
    modelLabel: "Tuteur Pédagogique Academy Lona (Mode Résilience)",
    isFallback: true,
...
function generateLocalPedagogicalFallback(query: string, courseContext?: any): string {
  ...
  if (q.includes("bonjour") || q.includes("salut") || q.includes("coucou")) {
    return `### Bonjour ! Bienvenue dans votre espace d'étude Academy Lona

Je suis votre assistant d'apprentissage intelligent de la Fondation Lona. Comment puis-je vous accompagner aujourd'hui ${courseTitle} ? 

Vous pouvez me demander :
- Des explications détaillées sur un concept clé ou un terme technique
- Un résumé synthétique des notions à retenir
- Des exemples pratiques appliqués aux priorités de développement en RDC
- De l'aide pour préparer un quiz ou valider votre certificat officiel Fondation Lona.`;
  }
...
1. **Identifiez les notions clés** : relisez les définitions et les protocoles opérationnels du module.
...
- **Compréhension du contexte** : reliez toujours les notions théoriques aux priorités sanitaires, sociales ou économiques de la RDC.
- **Rigueur méthodologique** : appliquez les normes, guides de procédure et critères d'excellence enseignés dans le cours.
- **Impact & Durabilité** : mesurez l'impact positif et l'utilité directe de votre apprentissage pour votre communauté.
...
1. **Notion fondamentale** : Dans le parcours académique Academy Lona, l'assimilation de cette compétence repose sur l'association entre rigueur scientifique et application pratique sur le terrain.
...
3. **Application pratique** : Identifiez l'impact direct dans vos activités quotidiennes ou au service de la société congolaise.
...
async function startServer() {
  ...
  app.use((req, res, next) => {
    ...
    if (isSocialCrawler && !req.path.startsWith("/api/") && !req.path.includes(".")) {
      ...
      const host = rawHost.includes("localhost") || !rawHost ? "academylona.cd" : rawHost;
      ...
      const title = "ACADEMY LONA - Fondation Lona | Excellence & Éducation en RDC";
      const description = "Plateforme d'apprentissage officielle de la Fondation Lona (Denise Nyakeru Tshisekedi) : bourses d'élite EXCELLENTIA (STEM), santé préventive, drépanocytose, autonomisation féminine et civisme.";
...
<head>
  ...
  <meta property="og:site_name" content="ACADEMY LONA">
  ...
  <meta property="og:image:alt" content="Academy Lona - Fondation Lona en RDC">
  ...
  <meta property="og:image:alt" content="Logo Academy Lona Officiel">

  <link rel="image_src" href="${ogJpgUrl}">

  <!-- Twitter / X -->
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:site" content="@fondationlona">
  <meta name="twitter:url" content="${appUrl}">
  <meta name="twitter:title" content="${title}">
  <meta name="twitter:description" content="${description}">
  <meta name="twitter:image" content="${ogJpgUrl}">
  <meta name="twitter:image:alt" content="Academy Lona - Fondation Lona en RDC">

  <link rel="icon" type="image/svg+xml" href="${baseUrl}/icon.svg">
  <link rel="icon" type="image/png" sizes="32x32" href="${baseUrl}/favicon-32x32.png">
  <link rel="apple-touch-icon" href="${baseUrl}/apple-touch-icon.png">
</head>
<body style="background:#0F172A;color:#ffffff;font-family:sans-serif;padding:30px;text-align:center;">
  <div style="max-width:600px;margin:0 auto;">
    <h1 style="color:#ffffff;margin-bottom:10px;">ACADEMY LONA</h1>
    <p style="color:#cbd5e1;font-size:15px;line-height:1.5;">${description}</p>
    <img src="${ogJpgUrl}" alt="Academy Lona Preview" style="max-width:100%;height:auto;border-radius:12px;margin:20px 0;box-shadow:0 10px 25px rgba(0,0,0,0.4);border:1px solid #183D73;">
    <div>
      <a href="${appUrl}" style="display:inline-block;background:#183D73;color:#ffffff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:bold;font-size:14px;border:1px solid #E8B84B;">Accéder à Academy Lona</a>
    </div>
  </div>
</body>
</html>`;
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Academy Lona Server running on http://0.0.0.0:${PORT}`);
  });
}
