...
const RECIPIENTS: Recipient[] = [
  ...
  { id: 'coord1', name: 'Coordination Pédagogique Fondation Lona', role: 'Coordination', avatar: '' },
...
const INITIAL_MESSAGES: Message[] = [
  ...
  { id: 2, senderId: 'sys', senderName: 'Système Academy Lona', senderRole: 'Système', avatar: '', text: 'Votre note de l\'Étude de Cas Pratique a été publiée : 9.5 / 10.', time: 'Hier', read: true, isMe: false },
...
const CoachAIChat = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { 
      id: 1, 
      role: 'ai', 
      text: 'Bonjour ! Je suis votre Assistant Pédagogique Academy Lona (Fondation Lona). 🤖\n\nJ\'ai analysé votre progression : vous avancez remarquablement ! Avez-vous besoin d\'un conseil pour aborder le prochain module ?',
  ...
  return (
    <>
      <button 
        ...
        className={`fixed bottom-24 right-6 z-[100] w-14 h-14 rounded-full shadow-2xl flex items-center justify-center transition-all duration-300 hover:scale-105 active:scale-95 border-2 border-white/20 cursor-pointer ${isOpen ? 'bg-[#E8B84B] text-[#0F172A] rotate-90' : 'bg-[#183D73] text-[#E8B84B]'}`}
      >
        {isOpen ? <X size={24} className="text-[#0F172A]" /> : <Sparkles size={24} className="text-[#E8B84B] fill-[#E8B84B]" />}
      </button>

      {isOpen && (
        <div className="fixed bottom-40 right-6 z-[99] w-[90vw] md:w-[380px] h-[480px] bg-[#1E293B] border border-[#183D73] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-6 duration-300">
          <div className="p-4 bg-[#0F172A] border-b border-white/10 flex items-center gap-3">
            <div className="w-9 h-9 bg-[#183D73]/60 rounded-full flex items-center justify-center">
              <Bot size={18} className="text-[#E8B84B]" />
            </div>
            <div>
              <h3 className="font-bold text-white text-xs uppercase tracking-wide">Assistant Pédagogique Lona IA</h3>
              <span className="text-[10px] text-emerald-400 font-bold flex items-center gap-1">
                <span className="w-1.5 h-1.5 bg-emerald-400 rounded-full animate-pulse" /> Academy Lona • En ligne
              </span>
            </div>
          </div>
