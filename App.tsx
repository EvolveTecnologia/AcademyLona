...
const App: React.FC = () => {
  ...
  const [downloadedIds, setDownloadedIds] = useState<string[]>(() => {
    const saved = localStorage.getItem('academylona_downloads') || localStorage.getItem('educaflix_downloads');
    return saved ? JSON.parse(saved) : [];
  });

  // Global My List (Favorites) State
  const [myListIds, setMyListIds] = useState<string[]>(() => {
    const saved = localStorage.getItem('academylona_mylist') || localStorage.getItem('educaflix_mylist');
    return saved ? JSON.parse(saved) : [];
  });
  ...
  useEffect(() => {
    localStorage.setItem('academylona_downloads', JSON.stringify(downloadedIds));
  }, [downloadedIds]);

  useEffect(() => {
    localStorage.setItem('academylona_mylist', JSON.stringify(myListIds));
  }, [myListIds]);
