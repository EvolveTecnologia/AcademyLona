...
export const GeminiStudyAssistant: React.FC<GeminiStudyAssistantProps> = ({
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      content: initialCourse 
        ? `Bonjour Placide ! Je suis votre **Tuteur IA Pédagogique Academy Lona**, dédié au cours **${initialCourse.title}** de la Fondation Lona.\n\nJe suis à votre entière disposition pour vous expliquer n'importe quelle leçon, clarifier des formules et concepts scientifiques, générer un quiz d'entraînement ou vous guider dans vos études en RDC.\n\nQuelle question souhaitez-vous aborder aujourd'hui ?`
        : `Bonjour Placide ! Je suis votre **Tuteur IA Pédagogique Academy Lona** (Fondation Lona).\n\nJe vous accompagne dans l'ensemble de vos cours : programme EXCELLENTIA & STEM, santé préventive (drépanocytose), entrepreneuriat féminin et droits humains.\n\nSélectionnez une formation ou posez directement votre question d'étude !`,
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      const fallbackMsg: Message = {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        content: `Pour votre question sur "${query}" :\n\nDans le cadre de votre progression sur Academy Lona (Fondation Lona), nous vous conseillons de réviser les notions clés de la leçon et de vérifier l'application concrète des concepts dans le contexte de la RDC.\n\n*N'hésitez pas à reformuler ou à préciser un point particulier du cours.*`,
        modelUsed: 'lona-tutor-offline-resilient',
        modelLabel: 'Tuteur Academy Lona (Continuité)',
        isFallback: true,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      };
  ...
  const suggestions = selectedCourse ? [
    ...
    `Comment appliquer cette formation dans le contexte de la RDC ?`,
    `Explique-moi les concepts les plus importants du Module 1.`
  ] : [
    `Comment réussir les examens du programme EXCELLENTIA ?`,
    `Quels sont les gestes clés pour la prévention de la drépanocytose ?`,
    `Comment préparer et réussir mon certificat officiel Academy Lona ?`,
    `Quelles sont les opportunités pour l'entrepreneuriat des femmes en RDC ?`
  ];
    ...
    <div className={`flex flex-col bg-[#122C34] text-white ${isFloating ? 'h-full max-h-[85vh] rounded-2xl border border-[#0A7A94]/40 shadow-2xl overflow-hidden' : 'h-full'}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-sm font-black uppercase tracking-wider text-white">Tuteur IA Pédagogique</h2>
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#E8B84B]/20 text-[#E8B84B] border border-[#E8B84B]/40">
                Officiel Academy Lona
              </span>
            </div>
      ...
      <div className="p-4 bg-[#1A2B32] border-t border-white/10 shrink-0">
        ...
        <div className="mt-2 flex items-center justify-between text-[10px] text-gray-400 px-1">
          ...
          <span className="font-semibold text-[#E8B84B]">Academy Lona AI</span>
        </div>
      </div>
